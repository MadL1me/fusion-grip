package lime.plugins.fusiongrip.logic
...
class AutoFdwSetupTask {
    fun setupFdwForSourceList(sources: List<IdeDataSource>, config: GenerationConfig): Pair<Boolean, String> {
        ...
        val local = LocalDbFactory.getLocalDb(LocalDbConstants.DEFAULT_DB_NAME)
        local.createFdwExtensionIfNotExists()

        val remoteServers = mutableListOf<RemoteDbServer>()
